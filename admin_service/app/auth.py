...
from . import schemas, crud, models  # Нужен crud
from .db import get_admin_db  # Сессия для Admin DB
...
# tokenUrl указывает на эндпоинт, где можно получить токен (наш /api/v1/auth/token)
...
async def get_current_active_admin(current_user: models.AdminUser = Depends(get_current_user)):
    if current_user.role != models.UserRolePythonEnum.ADMIN:
        raise HTTPException(
